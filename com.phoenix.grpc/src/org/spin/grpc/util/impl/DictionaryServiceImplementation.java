...
//	}
	...
	private Window.Builder convertWindow(Properties context, String uuid, boolean withTabs) {
		MWindow window = new Query(context, I_AD_Window.Table_Name, I_AD_Window.COLUMNNAME_AD_Window_UU + " = ?", null)
				.setParameters(uuid)
	...
	private Form.Builder convertForm(Properties context, String uuid) {
		MForm form = new Query(context, I_AD_Form.Table_Name, I_AD_Window.COLUMNNAME_AD_Window_UU + " = ?", null)
				.setParameters(uuid)
				.setOnlyActiveRecords(true)
				.first();
	...
	private Form.Builder convertForm(Properties context, MForm form) {
		...
		builder = Form.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(form.getAD_Form_UU()))
...
//		ContextInfo.Builder contextInfoBuilder = convertContextInfo(context, window.getAD_ContextInfo_ID());
		...
		builder = Window.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(window.getAD_Window_UU()))
...
//			List<MTab> tabs = ASPUtil.getInstance(context).getWindowTabs(window.getAD_Window_ID());
			...
			for(MTab tab : tabs) {
				...
				Tab.Builder tabBuilder = convertTab(context, tab, false);
...
//		MRecentItem.addMenuOption(Env.getCtx(), menuId, optionId);
	...
	private Tab.Builder convertTab(Properties context, String uuid, boolean withFields) {
		MTab tab = new Query(context, I_AD_Tab.Table_Name, I_AD_Tab.COLUMNNAME_AD_Tab_UU + " = ?", null)
				.setParameters(uuid)
				.setOnlyActiveRecords(true)
				.first();
		//	Convert
	...
	private Process.Builder convertProcess(Properties context, String uuid, boolean withParameters) {
		MProcess process = new Query(context, I_AD_Process.Table_Name, I_AD_Process.COLUMNNAME_AD_Process_UU + " = ?", null)
				.setParameters(uuid)
				.setOnlyActiveRecords(true)
				.first();
		//	Convert
...
//		}
		...
		if(tab.getTabLevel() > 0
			...
			if(tabList.size() == 0) {
				...
				if(optionalTab.isPresent()) {
					parentTabUuid = optionalTab.get().getAD_Tab_UU();
				}
			} else {
				...
				for(MTab currentTab : tabList) {
					...
					if(Util.isEmpty(parentTabUuid)) {
						parentTabUuid = currentTab.getAD_Tab_UU();
					}
				}
		...
		Tab.Builder builder = Tab.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(tab.getAD_Tab_UU()))
...
//		process = ASPUtil.getInstance(context).getProcess(process.getAD_Process_ID());
		Process.Builder builder = Process.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(process.getAD_Process_UU()))
...
//		}
	...
	private Field.Builder convertProcessParameter(Properties context, MProcessPara processParameter) {
		...
		Field.Builder builder = Field.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(processParameter.getAD_Process_Para_UU()))
...
//				.setIsInfoOnly(processParameter.isInfoOnly())
		...
		if(DisplayType.isLookup(displayTypeId)) {
			...
			MLookupInfo info = MLookupFactory.getLookupInfo(context, 0, 0, displayTypeId, Language.getLanguage(Env.getAD_Language(context)), columnName, referenceValueId, false, validationCode);
...
//	}
	...
	private Field.Builder convertField(Properties context, FieldRequest request) {
		...
		if(!Util.isEmpty(request.getFieldUuid())) {
			...
			MColumn column = new Query(context, I_AD_Column.Table_Name, I_AD_Column.COLUMNNAME_AD_Column_UU + " = ?", null)
					.setParameters(request.getColumnUuid())
					.setOnlyActiveRecords(true)
					.first();
			builder = convertField(context, column);
		} else if(!Util.isEmpty(request.getElementUuid())) {
			M_Element element = new Query(context, I_AD_Element.Table_Name, I_AD_Element.COLUMNNAME_AD_Element_UU + " = ?", null)
	...
	private Field.Builder convertField(Properties context, String uuid) {
		MField field = new Query(context, I_AD_Field.Table_Name, I_AD_Field.COLUMNNAME_AD_Field_UU + " = ?", null)
				.setParameters(uuid)
				.setOnlyActiveRecords(true)
				.first();
		//	Convert
	...
	private Field.Builder convertField(Properties context, MColumn column) {
		...
		Field.Builder builder = Field.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(column.getAD_Column_UU()))
...
//				.setIsRange(column.isRange())
		...
		if(DisplayType.isLookup(displayTypeId)) {
			...
			MLookupInfo info = MLookupFactory.getLookupInfo(context, 0, column.getAD_Column_ID(), displayTypeId, Language.getLanguage(Env.getAD_Language(context)), column.getColumnName(), referenceValueId, false, validationCode);
			if(info != null) {
				Reference.Builder referenceBuilder = convertReference(context, info);
				builder.setReference(referenceBuilder.build());
			} else {
	...
	private Field.Builder convertField(Properties context, M_Element element) {
		...
		MColumn column = new Query(context, MColumn.Table_Name, M_Element.COLUMNNAME_AD_Element_ID+"= ?", null)
		...
		Field.Builder builder = Field.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(element.getAD_Element_UU()))
		...
		if(DisplayType.isLookup(displayTypeId)) {
			...
			MLookupInfo info = MLookupFactory.getLookupInfo(context, 0, element.getAD_Element_ID(), displayTypeId, Language.getLanguage(Env.getAD_Language(context)), element.getColumnName(), referenceValueId, false, null);
			if(info != null) {
				Reference.Builder referenceBuilder = convertReference(context, info);
				builder.setReference(referenceBuilder.build());
			} else {
	...
	private Field.Builder convertField(Properties context, MField field, boolean translate) {
		...
		Field.Builder builder = Field.newBuilder()
				...
				.setUuid(ValueUtil.validateNull(field.getAD_Field_UU()))
...
//		}
	...
	private FieldGroup.Builder convertFieldGroup(Properties context, int fieldGroupId) {
		...
		if(fieldGroupId > 0) {
			...
			builder = FieldGroup.newBuilder()
					...
					.setUuid(ValueUtil.validateNull(fieldGroup.getAD_FieldGroup_UU()))
	...
	private Reference.Builder convertReference(Properties context, ReferenceRequest request) {
		...
		if(!Util.isEmpty(request.getReferenceUuid())) {
			X_AD_Reference reference = new Query(context, I_AD_Reference.Table_Name, I_AD_Reference.COLUMNNAME_AD_Reference_UU + " = ?", null)
			...
			if(reference.getValidationType().equals(X_AD_Reference.VALIDATIONTYPE_TableValidation)) {
				MLookupInfo info = MLookupFactory.getLookupInfo(context, 0, 0, DisplayType.Search, Language.getLanguage(Env.getAD_Language(context)), null, reference.getAD_Reference_ID(), false, null);
				if(info != null) {
			...
			MLookupInfo info = MLookupFactory.getLookupInfo(context, 0, 0, DisplayType.TableDir, Language.getLanguage(Env.getAD_Language(context)), request.getColumnName(), 0, false, null);
			if(info != null) {
				builder = convertReference(context, info);
			}
		}
	...
	private ValidationRule.Builder convertValidationRule(Properties context, ValidationRuleRequest request) {
		MValRule validationRule = new Query(context, I_AD_Val_Rule.Table_Name, I_AD_Val_Rule.COLUMNNAME_AD_Val_Rule_UU + " = ?", null)
				.setParameters(request.getValidationRuleUuid())
				.first();
		return ValidationRule.newBuilder()
				.setValidationRuleUuid(ValueUtil.validateNull(validationRule.getAD_Val_Rule_UU()))
