...
//		}
	...
	public List<MField> getWindowFields(int tabId) {
		MTab tab = new MTab(context, tabId, null);
	...
	private MProcess getProcessForASP(int processId) {
		...
		//	Save dictionary
		MProcess clonedProcess = MProcess.get(context, processId);
		processCache.put(getDictionaryKey(processId), clonedProcess);
		//	Old compatibility
...
//	}
	...
	private List<MProcessPara> loadProcessParameters(MProcess process) {
		...
		MProcessPara[] arrPara = process.getParameters();
		List<MProcessPara> para = new ArrayList<MProcessPara>();
		for(MProcessPara p: arrPara) {
			para.add(p);
		}
		processParameterCache.put(getDictionaryKey(process.getAD_Process_ID()), para);
		//	ASP Client
		parameters = para;
...
//		List<MWindow> customWindowList = getClientWindowList(window.getAD_Window_ID());
//		if(customWindowList != null
//				&& customWindowList.size() > 0) {
//			for(MWindow customWindow : customWindowList) {
