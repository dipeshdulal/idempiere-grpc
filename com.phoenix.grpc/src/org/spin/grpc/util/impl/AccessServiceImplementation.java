...
//		DB.validateSupportedUUIDFromDB();
		...
		if(isDefaultRole) {
			...
			roleId = DB.getSQLValue(null, "SELECT AD_Role_ID FROM AD_Role WHERE AD_Role_UU = ?", request.getRoleUuid());
			organizationId = DB.getSQLValue(null, "SELECT AD_Org_ID FROM AD_Org WHERE AD_Org_UU = ?", request.getOrganizationUuid());
			warehouseId = DB.getSQLValue(null, "SELECT M_Warehouse_ID FROM M_Warehouse WHERE M_Warehouse_UU = ?", request.getWarehouseUuid());
		}
		if(organizationId < 0) {
			organizationId = 0;
		}
		...
		builder.setUuid(ValueUtil.validateNull(session.getAD_Session_UU()));
...
//		DB.validateSupportedUUIDFromDB();
	...
	private UserInfo.Builder convertUserInfo(MUser user) {
		...
		userInfo.setUuid(ValueUtil.validateNull(user.getAD_User_UU()));
	...
	private UserInfoValue.Builder convertUserInfoFromSession(UserInfoRequest request) {
		...
		MSession session = new Query(Env.getCtx(), I_AD_Session.Table_Name, I_AD_Session.COLUMNNAME_AD_Session_UU + " = ?", null)
				.setParameters(request.getSessionUuid())
				.first();
		if(session == null
				|| session.getAD_Session_ID() <= 0) {
	...
	private Role.Builder convertRole(MRole role, boolean withAccess) {
		...
		if(role != null) {
			...
			builder = Role.newBuilder()
					...
					.setUuid(ValueUtil.validateNull(role.getAD_Role_UU()))
			...
			if(withAccess) {
				...
				for(MRoleOrgAccess access : orgAccessList) {
					...
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(organization.getAD_Org_UU()))
				...
				for(MProcessAccess access : processAccessList) {
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(access.getAD_Process().getAD_Process_UU()))
				...
				for(MWindowAccess access : windowAccessList) {
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(access.getAD_Window().getAD_Window_UU()))
							.setIsReadOnly(!access.isReadWrite());
				...
				for(MFormAccess access : formAccessList) {
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(access.getAD_Form().getAD_Form_UU()))
							.setIsReadOnly(!access.isReadWrite());
...
//				}
				...
				for(X_AD_Task_Access access : taskAccessList) {
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(access.getAD_Task().getAD_Task_UU()))
							.setIsReadOnly(!access.isReadWrite());
...
//				}
				...
				for(MWorkflowAccess access : workflowAccessList) {
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(access.getAD_Workflow().getAD_Workflow_UU()))
							.setIsReadOnly(!access.isReadWrite());
				...
				for(X_AD_Document_Action_Access access : documentActionAccessList) {
					...
					Access.Builder accessBuilder = Access.newBuilder()
							.setUuid(ValueUtil.validateNull(documentType.getC_DocType_UU()))
...
//							.setIsCanExport(access.isCanExport())
	...
	private Menu.Builder convertMenu(Properties context, String language) {
		...
		if (treeId <= 0) {
			treeId = MTree.getDefaultAD_Tree_ID(menu.getAD_Client_ID(), I_AD_Menu.COLUMNNAME_AD_Menu_ID);
//			treeId = MTree.getDefaultTreeIdFromTableId(menu.getAD_Client_ID(), I_AD_Menu.Table_ID);
		}
		if(treeId != 0) {
			MTree tree = new MTree(Env.getCtx(), treeId, false, false, null, null, -1);
//			MTree tree = new MTree(Env.getCtx(), treeId, false, false, null, null);
			...
			while (childrens.hasMoreElements()) {
				...
				MMenu mmenu = new Query(context, I_AD_Menu.Table_Name, I_AD_Menu.COLUMNNAME_AD_Menu_ID + " = " + child.getNode_ID(), null).first();
				Menu.Builder childBuilder = convertMenu(context, mmenu, child.getParent_ID(), language);
				//	Explode child
	...
	private Menu.Builder convertMenu(Properties context, MMenu menu, int parentId, String language) {
		...
		if(parentId > 0) {
			MMenu parentMenu = new Query(context, I_AD_Menu.Table_Name, I_AD_Menu.COLUMNNAME_AD_Menu_ID + " = " + parentId, null).first();
			parentUuid = parentMenu.getAD_Menu_UU();
		}
		Menu.Builder builder = Menu.newBuilder()
				.setId(menu.getAD_Menu_ID())
				.setUuid(ValueUtil.validateNull(menu.getAD_Menu_UU()))
...
//			}
	...
	private void addChildren(Properties context, Menu.Builder builder, MTreeNode node, String language) {
		...
		while (childrens.hasMoreElements()) {
			MTreeNode child = (MTreeNode)childrens.nextElement();
			MMenu menu = new Query(context, I_AD_Menu.Table_Name, I_AD_Menu.COLUMNNAME_AD_Menu_ID + " = " + child.getNode_ID(), null).first();
//			Menu.Builder childBuilder = convertMenu(context, MMenu.getFromId(context, child.getNode_ID()), child.getParent_ID(), language);
			Menu.Builder childBuilder = convertMenu(context, menu, child.getParent_ID(), language);
			addChildren(context, childBuilder, child, language);
			builder.addChilds(childBuilder.build());
		}
	}
